...
DEFAULT_TOPICS = [
    ...
    "window functions: ROW_NUMBER, RANK, and frame clauses",
    "recursive CTEs for graph and tree traversal",
    "DISTINCT ON for top-N-per-group queries",
    "GROUPING SETS, CUBE, and ROLLUP for multi-level aggregation",
    "INSERT ON CONFLICT (UPSERT) patterns and performance",
    "MERGE statement (Postgres 15+) for conditional DML",
    "extended statistics (CREATE STATISTICS) for correlated columns",
    "row-level security (RLS) for multi-tenant data isolation",
    "generated columns: stored vs virtual trade-offs",
    "full-text search with tsvector, tsquery, and GIN indexes",
    "advisory locks for application-level coordination",
    "TOAST storage and large column strategies",
    "pg_repack for online table reorganization without locks",
    "logical decoding for Change Data Capture (CDC)",
    "shared_buffers tuning and buffer cache hit ratio",
    "WAL archiving and Point-in-Time Recovery (PITR)",
    "connection lifecycle: idle-in-transaction timeout pitfalls",
    "pg_stat_statements: identifying top queries by total time",
    "sequence caching and performance under contention",
    "JSON path queries (SQL/JSON standard in Postgres 16+)",
    "common anti-patterns: SELECT * in production queries",
    "transaction ID wraparound and VACUUM FREEZE urgency",
    "trigger performance implications and WHEN clauses",
    "table access method: heap vs columnar extensions"
]
...
DATA_ENGINEERING_TOPICS = [
    ...
    "dbt incremental models: merge vs insert_overwrite strategies",
    "dbt snapshots for SCD Type 2 without manual SQL",
    "dbt macros and Jinja templating for DRY pipelines",
    "Medallion architecture: bronze, silver, gold layer design",
    "CDC with Debezium: capturing row-level changes in real time",
    "Data contracts: enforcing schema agreements between producers and consumers",
    "Delta Lake vs Apache Iceberg vs Apache Hudi trade-offs",
    "Kimball dimensional modeling: star schemas and conformed dimensions",
    "Data vault modeling: hubs, satellites, and links",
    "Late-arriving facts and dimension handling strategies",
    "Exactly-once vs at-least-once semantics in streaming pipelines",
    "Apache Kafka: topic partitioning, consumer groups, and offset management",
    "Spark optimization: broadcast joins, AQE, and partition pruning",
    "Reverse ETL: pushing warehouse data back to operational tools",
    "Data observability: detecting freshness, volume, and schema anomalies",
    "Apache Arrow and in-memory columnar processing",
    "Feature stores: bridging data engineering and ML pipelines",
    "Data mesh: domain ownership and self-serve data infrastructure",
    "Temporal tables and bi-temporal modeling patterns",
    "Cost optimization in Snowflake/BigQuery/Redshift",
    "ELT vs ETL: when transformation order matters",
    "Testing data pipelines: unit tests, data quality gates, and contract tests",
    "Orchestration patterns: Airflow vs Dagster vs Prefect trade-offs",
    "Data anonymization and PII handling in pipelines",
    "Slowly Changing Dimensions Types 3, 4, and 6",
    "Event-driven architecture and event sourcing for data systems",
    "Schema registries: Confluent, AWS Glue, and Avro compatibility modes",
    "Data catalog and metadata management (DataHub, OpenMetadata)",
    "Spark structured streaming: watermarks and late data handling",
    "Snowflake clustering keys vs Iceberg sort orders",
    "Data freshness SLAs: measuring and alerting on pipeline lag",

    "Hub-and-spoke data platform design: enforcing upstream-only dependency flow",
    "Materialized view refresh strategies at scale (100+ views)",
    "Drift detection: measuring data quality shifts between pipeline runs",
    "Circuit breaker pattern for resilient API ingestion pipelines",
    "KubernetesPodOperator: running ETL jobs as isolated Kubernetes pods",
    "Airflow DAG generation from YAML configs and Jinja2 templates",
    "Multi-database routing in Django: reader vs writer separation",
    "Pydantic for pipeline schema validation and data contracts",
    "LLM-assisted data analysis: prompt versioning and cost control",
    "Structured JSON logging for pipeline observability and debugging",
    "S3 as a staging layer between ETL pipeline steps",
    "Job auto-discovery patterns for scalable pipeline registration",
    "Multi-source API ingestion: normalizing GitHub, Jira, and Slack data",
    "Parquet column mapping with contract validation",
    "Data retention enforcement and lifecycle policies in pipelines",
    "Stability scoring: quantifying data quality over time",
    "Exponential backoff and retry strategies for API rate limits",
    "Thread-pool parallelism in Python ETL jobs (concurrent.futures)",
    "LocalStack for local AWS integration testing of S3/SQS pipelines",
    "Anomaly detection in time-series pipeline metrics (statsmodels)",
    "Cross-database references without foreign keys: ID-only patterns",
    "DAG distribution via S3 sidecar sync for Airflow on Kubernetes",
    "Django ORM in data pipelines: when to use models vs raw SQL",
    "Dependency injection in Python ETL: testable service composition",
    "Health scoring algorithms: combining signals into composite metrics",
    "Aggregation job design: daily, weekly, and sliding window patterns",
    "Pipeline idempotency with date-partitioned state files",
    "AWS Bedrock integration: calling Claude from data pipelines",
    "Code ownership boundaries in multi-repo data platforms",
    "Docker image staging patterns for spoke-based deployments"
]
